...
describe 'hashketball' do
  describe '#game_hash' do
    let(:top_level_keys) { %i[home away] }
    let(:team_level_keys) { %i[team_name colors players] }
    ...
    it 'returns the correct top-level keys' do
      top_level_keys.each { |key| expect(game_hash.keys).to include(key) }
    end
  ...
  describe '#team_colors' do
    it 'knows the Brooklyn Nets colors are Black and White' do
      ...
      expect(team_colors('Charlotte Hornets')).to contain_exactly(
                  'Turquoise',
                  'Purple'
                )
    end
